param(
    ...
    [string]$AniScaleModelDirectory = "$env:USERPROFILE\Documents\apps\TheAnimeScripter\weights\aniscale2-onnx"
)
...
$engineSources = @{
    'AnimeSR_v2_fp16_op20_fp16_720x1280.engine' = Join-Path $ModelDirectory 'AnimeSR_v2_fp16_op20_fp16_720x1280.engine'
    'AnimeSR_v2_fp16_op20_fp16_1080x1920.engine' = Join-Path $ModelDirectory 'AnimeSR_v2_fp16_op20_fp16_1080x1920.engine'
    '2x_AniScale2S_Compact_i8_60K-fp16_fp16_1080x1920.engine' = Join-Path $AniScaleModelDirectory '2x_AniScale2S_Compact_i8_60K-fp16_fp16_1080x1920.engine'
}
...
foreach ($engineName in $engineSources.Keys) {
    $engineSource = $engineSources[$engineName]
    if (-not (Test-Path -LiteralPath $engineSource -PathType Leaf)) {
        throw "TensorRT engine was not found at $engineSource"
    }
}

& $PythonPath -c "import ast; ast.Str = getattr(ast, 'Str', ast.Constant); import torch, tensorrt, simple_websocket, werkzeug, PyNvVideoCodec" | Out-Null
if ($LASTEXITCODE -ne 0) {
    throw 'Python runtime is missing torch, TensorRT, PyNvVideoCodec, simple-websocket, or Werkzeug.'
}
...
foreach ($engineName in $engineSources.Keys) {
    Copy-Item -LiteralPath $engineSources[$engineName] -Destination $modelRoot -Force
}
